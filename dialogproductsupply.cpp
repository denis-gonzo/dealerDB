...
void DialogProductSupply::finishOnAccept()
{
    QMessageBox msg;
    if(current_id == -1){
        msg.setText("Добавить новую поставку товара?");
    } else {
        msg.setText("Вы уверены, что хотите изменить поставку?");
    }
    ...
    if( current_id != -1 && il.size() < 1 ){
        ...
        msg.setText("Ошибка, не удалось найти поставку для изменения.");
